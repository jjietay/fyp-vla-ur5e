...
Reference for the working install on the Legion Pro 5i. URSim was built for Ubuntu 14.04 / JDK 1.8, so getting it running on a modern stack requires several patches. This document captures the final state.
...
| What | Where |
|---|---|
| URSim root (`$URSIM`) | `~/Documents/NTU/Y4S1/FYP/ursim/URSim_Linux-5.25.2.130406/ursim-5.25.2.130406/` |
...
```bash
cd ~/Documents/NTU/Y4S1/FYP/ursim/URSim_Linux-5.25.2.130406/ursim-5.25.2.130406
./start-ursim.sh UR5e
```

Other model flags: `UR3e`, `UR10e`, `UR16e`, `UR20`, `UR30`. Pick the one that matches the lab arm (UR5e for the FYP).
...
- **Don't move the URSim directory** after install — the dynlibs symlink is absolute. If you must move it, update the symlink target.
