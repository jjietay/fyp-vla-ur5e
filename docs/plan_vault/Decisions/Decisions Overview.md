---
tags: [overview]
---

# Decisions Overview

Choices that shape the project, each with the reasoning attached. These become the methodology chapter of [[D4 Final Report]], so writing the reason down at the time is worth more than it seems.

| Decision | State |
|---|---|
| [[Camera Setup]] | decided, procurement pending |
| [[Speech Stack]] | decided |
| [[Detector Query Format]] | decided, forced by how OWLv2 works |
| [[Teleoperation Method]] | decided, hardware pending |
| [[Action Space]] | pending, needed by W5 |
| [[Calibration Marker]] | **pending since July, blocking W3** |

A decision recorded without its reason is not much use six months later when an examiner asks why.
